...
static void *morecore(int increment);
...
#define SPIN_LOCK_YIELD  SleepEx(SLEEP_EX_DURATION, veFalse)
...
static void *morecore(int increment)
{
	static void *previous_top;

	if (increment) {

		void *tmp;
		if (!(tmp = adl_memGet(increment + MALLOC_ALIGNMENT)))
			return (void*)-1;

		/* Must be aligned. */
		tmp = (void*) (((int)tmp + CHUNK_ALIGN_MASK) & ~CHUNK_ALIGN_MASK);
		previous_top = (void*)((int) tmp + increment + MALLOC_ALIGNMENT);
		return tmp;
	}
...
static void *morecore(int increment)
{
	...
	if (increment) {
		...
		void *tmp;
		if (!(tmp = malloc(increment + MALLOC_ALIGNMENT)))
			return (void*)-1;
